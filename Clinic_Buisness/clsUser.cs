...
namespace Clinic_Buisness
{
    public class clsUser
    {
        ...
        public static clsUser FindByPersonID(int PersonID)
        {
            ...
            if (IsFound)
                // FIX: Successfully passing PersonID here instead of UserID twice!
                return new clsUser(UserID, PersonID, UserName, Password, IsActive);
            else
                return null;
        }
        ...
        // Added to expose the password utility to the frontend UI
        public bool ChangePassword(string NewPassword)
        {
            if (this.UserID == -1) return false;

            bool isUpdated = clsUserData.ChangePassword(this.UserID, NewPassword);
            if (isUpdated)
            {
